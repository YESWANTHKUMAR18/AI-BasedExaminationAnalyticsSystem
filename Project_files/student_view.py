...
def show_student_dashboard(username):
    ...
    total_predictions = len(user_preds) if user_preds else 0
    ...
    avg_score_val = round(total_score_sum / total_predictions, 1) if total_predictions > 0 else 0
    pass_rate_val = round((passes / total_predictions) * 100, 1) if total_predictions > 0 else 0
    
    avg_dict = {k: (sums[k]/counts[k] if counts[k] else 0) for k in sums}
    top_sub = max(avg_dict, key=avg_dict.get) if total_predictions > 0 else "N/A"
    ...
    with m1:
        st.markdown(f'<div class="metric-card-new"><div class="label-new">My Predictions</div><div class="value-new">{total_predictions}</div></div>', unsafe_allow_html=True)
    with m2:
        st.markdown(f'<div class="metric-card-new"><div class="label-new">Avg. Accuracy</div><div class="value-new">{avg_score_val}%</div></div>', unsafe_allow_html=True)
    with m3:
        p_color = "#10B981" if pass_rate_val >= 50 else "#EF4444"
        st.markdown(f'<div class="metric-card-new"><div class="label-new">Pass Likelihood</div><div class="value-new" style="color: {p_color}">{pass_rate_val}%</div></div>', unsafe_allow_html=True)
    with m4:
        st.markdown(f'<div class="metric-card-new"><div class="label-new">Best Subject</div><div class="value-new" style="color: #6366F1">{top_sub}</div></div>', unsafe_allow_html=True)
    ...
    with c1:
        ...
        fig.update_layout(
            ...
            height=380, 
            yaxis=dict(range=[0, 115], gridcolor="#F1F5F9", title="Marks (0-100)"), 
    ...
    with c2:
        ...
        if total_predictions > 0:
            fig_pie = px.pie(
                names=["Pass", "Fail"], 
                values=[passes, total_predictions - passes], 
            ...
            st.info("Start a prediction to see your history.")
